...
export function CheckupUpsell() {
  ...
  if (isMatching) {
    return (
      <div className="mt-5 border-t border-line/60 pt-4">
        <div className="flex items-center justify-center gap-2 rounded-lg bg-mint-50 px-3 py-6 text-sm text-mint-800">
  ...
  if (error && !checkup) {
    return (
      <div className="mt-5 border-t border-line/60 pt-4">
        <div className="flex flex-col items-center gap-2 rounded-lg bg-amber-50 px-3 py-5 text-center ring-1 ring-amber-200">
          <AlertTriangle className="size-5 text-amber-700" />
          <div className="text-xs text-amber-800">แนะนำโปรแกรมไม่สำเร็จ · {error}</div>
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-mint-300/60 bg-gradient-to-b from-mint-50/80 to-white p-4">
        <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-mint-100 px-2.5 py-1 text-[11px] text-mint-800">
          <Sparkles className="size-3" />
          AI จับคู่จากผลปรึกษา
        </div>

        <p className="text-sm font-medium text-mint-900">{checkup.headline_th}</p>

        <div className="mt-3 rounded-xl border border-line/60 bg-white/70 p-3.5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h3 className="mt-1 text-sm font-semibold text-ink">{program.name_th}</h3>
              <p className="text-xs text-muted-foreground">
                {program.name_en} · {program.duration_th}
              </p>
            </div>
            <div className="shrink-0 text-right">
              <div className="text-base font-bold text-mint-700">
                {program.price.toLocaleString('th-TH')}฿
              </div>
              <span className="rounded-full bg-mint-100 px-1.5 py-0.5 text-[10px] text-mint-800">
        ...
        <p className="mt-3 flex gap-2 rounded-md bg-mint-50 px-3 py-2 text-xs text-mint-900">
          <Stethoscope className="mt-0.5 size-3.5 shrink-0 text-mint-700" />
          <span>{checkup.reason_th}</span>
        </p>

        {booked ? (
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-mint-400/60 bg-mint-100/70 px-3 py-2">
            <CalendarCheck className="mt-0.5 size-4 text-mint-800" />
            <div className="text-xs text-mint-900">
