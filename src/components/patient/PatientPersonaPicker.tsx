  ...
  return (
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-6 py-12 md:px-12 md:py-16">
      <div className="text-center">
        ...
        <h1 className="text-2xl font-bold text-foreground md:text-3xl">เลือกบทบาทผู้ป่วย</h1>
        ...
        {PATIENT_PERSONA_PRESETS.map(({ id, persona, label_en }) => (
          ...
          >
            <GlassCard className="flex h-full flex-col gap-2 p-5 transition-all group-hover:-translate-y-1 group-hover:shadow-lg">
              <div className="inline-flex size-12 items-center justify-center rounded-2xl bg-mint-100 dark:bg-mint-500/15 text-mint-700 dark:text-mint-400">
                <UserRound className="size-6" />
              </div>
              <div>
                <div className="text-base font-semibold text-foreground">{persona.name}</div>
              ...
              <div className="mt-1 flex flex-col gap-1 text-[11px] text-muted-foreground">
                {persona.conditions.length ? (
                  <div>โรคประจำตัว: <span className="text-foreground">{persona.conditions.join(', ')}</span></div>
                ) : (
                  <div className="text-muted-foreground/70">ไม่มีโรคประจำตัว</div>
                )}
                {persona.allergies.length ? (
                  <div>แพ้: <span className="text-foreground">{persona.allergies.join(', ')}</span></div>
                ) : null}
              </div>

              <div className="mt-auto flex items-center justify-end border-t border-border/40 pt-3 text-[11px] font-medium text-mint-700 dark:text-mint-400">
                เริ่มใช้งาน
